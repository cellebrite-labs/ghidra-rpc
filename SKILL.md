...
| `ghidra-rpc disassemble <binary> <address> [--count N] [--verbose-list]` | Disassemble N instructions (default 20); returns listing text (pass `--verbose-list` to include details about every instruction); `warning` field if start address had no instruction | `{address, count, listing[, instructions][, warning]}` |
